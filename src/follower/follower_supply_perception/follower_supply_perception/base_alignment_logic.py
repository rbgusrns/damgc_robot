"""ROS-independent alignment decisions for Follower base-frame tag samples."""
...
class BaseAlignmentThresholds:
    ...
    def validate(self) -> None:
        """Raise ``ValueError`` when thresholds cannot define valid states."""
        values = (
            self.target_forward,
...
class BaseAlignmentMeasurement:
    """One timestamped planar sample expressed in ``base_link``."""
...
def _is_above(value: float, boundary: float) -> bool:
    """Test an upper boundary while accepting rounded equivalents."""
...
def _is_below(value: float, boundary: float) -> bool:
    """Test a lower boundary while accepting rounded equivalents."""
    return value < boundary and not isclose(
        value, boundary, rel_tol=1.0e-9, abs_tol=1.0e-12
    )
...
class BaseAlignmentStateMachine:
    """Evaluate one coherent sample and track continuous stability."""
    ...
    def update(
        ...
        """Return the highest-priority base alignment state."""
        ...
        if _is_above(
            ...
            self._thresholds.target_forward + self._thresholds.forward_tolerance,
        ):
