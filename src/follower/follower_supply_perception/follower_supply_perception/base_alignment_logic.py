"""ROS-independent hybrid alignment decisions for Follower tag samples."""
...
class ControlMode(str, Enum):
    """Internal control phases carried by the atomic alignment command."""
...
class BaseAlignmentThresholds:
    """Follower thresholds for visibility-first tag-normal alignment."""
    ...
    def validate(self) -> None:
        ...
        if self.final_forward_tolerance <= 0.0:
            raise ValueError("final_forward_tolerance must be positive")
        if self.final_lateral_tolerance <= 0.0:
            raise ValueError("final_lateral_tolerance must be positive")
...
class BaseAlignmentMeasurement:
    """One coherent center/normal geometry sample in ``base_link``."""
...
class AlignmentDecision:
    """External state, internal mode, and active planar control target."""
...
def compute_near_control(
    ...
    requested = normalize_angle(prealign_bearing - tag_bearing)
    effective_limit = correction_limit * visibility_scale
    correction = max(-effective_limit, min(effective_limit, requested))
    return NearControl(
        steering_error=normalize_angle(tag_bearing + correction),
...
def compute_blind_remaining_distance(
    ...
    """Return a finite forward-only blind distance within its hard limit."""
    values = (
        last_valid_tag_x,
...
def is_blind_final_approach_eligible(
    *,
    ...
    """Gate blind motion to one recent, close, final visual sample."""
    ...
    if (
        ...
        return False
    age = receipt_now - last_valid_receipt
    ...
    return compute_blind_remaining_distance(
        last_valid_tag_x, final_target_distance, max_distance
    ) is not None
...
def compute_forward_progress(
    ...
    if not all(isfinite(value) for value in values):
        return None
    return cos(start_yaw) * (current_x - start_x) + sin(start_yaw) * (
        current_y - start_y
    )


class BaseAlignmentStateMachine:
    """Hybrid FAR/NEAR/final state machine with visibility hysteresis."""
    ...
    def update(
        ...
        if not self._final_phase:
            ...
            if prealign_error <= self._thresholds.pre_align_position_tolerance:
                ...
                if measurement.final_x >= -self._thresholds.final_forward_tolerance:
                    self._final_phase = True
                else:
                ...
                near = compute_near_control(
                    tag_bearing,
                    atan2(measurement.prealign_y, measurement.prealign_x),
    ...
    def _coarse_decision(
        ...
        if self._turn_direction and (
            self._turn_direction * tag_bearing <= self._thresholds.turn_exit_error_rad
        ):
            self._turn_direction = 0
        ...
        return self._leave_stable(
            AlignmentDecision(
                state, ControlMode.COARSE_TRACK, measurement.tag_x, measurement.tag_y
            )
        )
    ...
    def _recenter_decision(
        ...
        return self._leave_stable(
            AlignmentDecision(
                state, ControlMode.RECENTER, measurement.tag_x, measurement.tag_y
            )
        )
    ...
    def _final_decision(
        ...
        position_good = (
            abs(measurement.final_x) <= self._thresholds.final_forward_tolerance
            and abs(measurement.final_y) <= self._thresholds.final_lateral_tolerance
        )
        if not position_good:
            ...
            return self._leave_stable(
                AlignmentDecision(
                    ...
                    max(0.0, measurement.final_x) * visibility.forward_scale,
