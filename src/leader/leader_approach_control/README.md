...
Leader의 tag-normal 기반 control target pose와 alignment state를 사용해
...
Controller는 target pose 직후 `sample_sync_tolerance` 안에 도착한 state만 결합한다.
Stamp/receipt timeout, ID 변경, invalid pose, unknown state 또는 disabled gate에서는
...
|---|---:|---:|
| `TURN_LEFT/RIGHT` | 0 | pre-align target bearing |
| `APPROACH` | positive, 최대 `0.05 m/s` | pre-align bearing correction |
| `FINE_ALIGN_LEFT/RIGHT` | 0 | final target yaw error |
...
실차 단계별 검증은
[`LEADER_TAG_NORMAL_ALIGNMENT_VALIDATION_GUIDE.md`](../rescue_robot_apriltag/docs/LEADER_TAG_NORMAL_ALIGNMENT_VALIDATION_GUIDE.md)를 따른다.
